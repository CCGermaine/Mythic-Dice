...
The extension stores the token id on the player under `com.mythic-dice/attachedTokenId`, and the chosen color under `com.mythic-dice/dieColor`. It does not set the die's `attachedTo` field. `attachedTo` would parent the die to the token and move it whenever the token moves. The die is a Prop image placed at 175 pixels from the token's center, on a random angle around that circle. Each click adds another die. Colors are white, red, blue, green, and black. The blue art is the teal pigment in the files.
...
The roll item starts as `{die}_{color}_rolling.webm` (`video/webm`) and, after 1.5 seconds, the same item becomes `{die}_{color}_{NN}_flat.webp` (`image/webp`). The face is a random number from 1 through that die's highest face.
